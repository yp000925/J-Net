'''
This is adapted from swin unet for denoising, different upsampling method
'''
import torch
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm):
        ...
        self.input_resolution = input_resolution
        self.dim = dim
        self.reduction = nn.Linear(4 * dim, 2 * dim, bias=False) # params
...
class UpSample(nn.Module):
    def __init__(self, input_resolution, in_channels, scale_factor):
        ...
        if self.factor == 2:
            ...
            self.conv = nn.Conv2d(2*in_channels, in_channels, 1, 1, 0, bias=False)
            self.up_p = nn.Sequential(nn.Conv2d(in_channels, 64 * in_channels, 1, 1, 0, bias=False),
                                      nn.PReLU(),
                                      nn.PixelShuffle(scale_factor),
                                      nn.Conv2d(in_channels, in_channels, 1, stride=1, padding=0, bias=False))
...
class SNet(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1,
                 ...
                 use_checkpoint=False, final_head=None, **kwargs):
        ...
        # if self.final_head == "with activation":
        #     self.act = nn.Sigmoid()
        # else:
        #     self.act = nn.Identity()
        self.apply(self._init_weights)
        if self.final_head == "sigmoid":
            self.act = nn.Sigmoid()
        elif self.final_head == "identity":
            self.act = nn.Identity()
    ...
    def forward(self, x):
        ...
        x = torch.flatten(x,1)  # B, C, L
        x = self.head(x)
        if self.final_head == 'clip':
...
if __name__ == '__main__':
    ...
    model_D_config = model_config['MODEL_D']['PARAMS']
    model = SNet(**model_D_config)
    # model = SNet(img_size=512, patch_size=8, in_chans=1, out_chans=1,
    #               embed_dim=48, depths=[2, 2, 2, 2],
    #               num_heads=[2, 2, 4, 4],
    #               window_size=16, mlp_ratio=4., qkv_bias=True, qk_scale=2,
    #               drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
    #               norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
    #               use_checkpoint=False,final_head='sigmoid')  # .cuda()
    # print(model)
