...
def generate_otf_torch(wavelength, nx, ny, deltax, deltay, distance, pad_size=None, device='cpu'):
    ...
    deltax = torch.tensor(deltax).to(device)
    deltay = torch.tensor(deltay).to(device)
...
class AdpNetDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'C':
            ...
            phase_img = self.rescale_phase(phase_img, range=[0, 2*np.pi])
            # if self.pad_size:
            #     phase_img = self.zero_pad_image(phase_img, self.pad_size)
...
if __name__ == "__main__":
    ...
    with open('/Users/zhangyunping/PycharmProjects/PATrans/configs/CleePolySample.yaml') as f:
        config = yaml.load(f, Loader=yaml.FullLoader)
    prop_kernel = config['MEASUREMENT']['prop_kernel']
    prop_kernel['pad_size'] = [512, 512]

    amp_img = np.array(Image.open("/Users/zhangyunping/PycharmProjects/PATrans/ExpSample/CLee/poly/test/gt_amplitude.png").convert('L')).astype('float32') / 255.0
    # if self.pad_size:
    #     amp_img = self.one_pad_image(amp_img, self.pad_size)
    phase_img = np.zeros_like(amp_img)
    comp_field = amp_img * np.exp(1j * phase_img)
    operator = DH_operator(**prop_kernel)
    holo = operator.forward(torch.tensor(comp_field))
    ...
    bp_amp = bp.abs()
    bp_phase = bp.angle()
    plt.imsave('simholo3.png', holo, cmap='gray')
    plt.imsave('bp_amp3.png', bp_amp, cmap='gray')
    plt.imsave('bp_phase3.png', bp_phase, cmap='gray')
    plt.imsave('bp_phase_c3.png', bp_phase, cmap='hot')
    # fs_out = torch.multiply(torch.fft.fft2(comp_field), A.expand(comp_field.shape))
    # f_out = torch.fft.ifft2(fs_out)
    #
    # y = y.unsqueeze(0)
    # holo = y[0, 0, :, :]
