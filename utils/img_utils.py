...
def total_variation_tensor(img):
    assert len(img.shape) == 4
    diff_w = img[:, :, :, :-1] - img[:, :, :, 1:]
    diff_h = img[:, :, :-1, :] - img[:, :, 1:, :]
    # Calculate squared differences
    diff_w_squared = torch.pow(diff_w, 2)
    diff_h_squared = torch.pow(diff_h, 2)

    # Sum the squared differences and take the square root
    tv = torch.sqrt(torch.sum(diff_w_squared) + torch.sum(diff_h_squared))
    return tv
